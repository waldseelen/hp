...
from apps.main.file_validators import FileTypeValidator, ImageValidator, SecureFileValidator
...
def test_contact_form_validation():
    ...
    if form.is_valid():
        ...
        assert '<img' not in clean_data['subject'], "HTML should be stripped from subject"
        assert '<script>' not in clean_data['message'], "XSS should be stripped from message"
    ...
    assert 'website' in form.errors or any('spam' in str(errors).lower() for errors in form.errors.values())
...
def test_csrf_protection():
    ...
    try:
        ...
        assert response.status_code in [403, 302], f"Expected 403 or 302, got {response.status_code}"
    ...
    try:
        ...
        assert response.status_code in [403, 302], f"API should require CSRF, got {response.status_code}"
