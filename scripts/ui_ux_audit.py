...
class UIUXAuditor:
    ...
    def audit_color_consistency(self):
        ...
        for html_file in self.html_files:
            try:
                ...
                if len(inline_styles) > 5:  # Threshold
                    self.issues.append(
                        f"⚠️  Excessive inline styles in {html_file} ({len(inline_styles)} found)"
                    )
        ...
        if inline_style_count > 20:
            self.issues.append(
                "⚠️  Too many inline styles found - should use CSS classes instead"
            )
    ...
    def audit_typography_consistency(self):
                ...
                if font_families:
                    for font in font_families:
                        if "Inter" not in font and "sans-serif" not in font:
                            self.issues.append(
                                f"⚠️  Non-standard font found in {html_file}: {font}"
                            )
        ...
        if typography_usage["headings_heading-1"] < 1:
            self.issues.append(
                "⚠️  Design system heading classes not being used consistently"
            )
        ...
        for html_file in self.html_files:
            try:
                ...
                if len(arbitrary_spacing) > 3:
                    self.issues.append(
                        f"⚠️  Too many arbitrary spacing values in {html_file} ({len(arbitrary_spacing)} found)"
                    )
                ...
                for button in button_elements:
                    if not any(
                        ...
                        if "btn-" not in button:  # Should use design system
                            self.issues.append(
                                f"⚠️  Button without design system class in {html_file}"
                            )
        ...
        for html_file in self.html_files:
            try:
                ...
                if "motion-reduce:" not in content and (
                    ...
                    if motion_classes > 3:
                        self.issues.append(
                            f"⚠️  Missing motion-reduce variants in {html_file} (has {motion_classes} animations)"
                        )
            ...
            try:
                ...
                for element in required_pwa_elements:
                    if element not in content:
                        self.issues.append(
                            f"⚠️  Missing PWA element in base template: {element}"
                        )
                    else:
