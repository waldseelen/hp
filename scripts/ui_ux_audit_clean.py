...
class UIUXAuditor:
    def __init__(self, project_root: Path):
        ...
        self.design_system = {
            'colors': {
                ...
                'secondary': ['secondary-', 'text-secondary', 'bg-secondary', 'border-secondary'],
                'brand': ['#e6c547', '#0f172a'],  # Brand colors from Phase 7
                'dark_theme': ['dark:', 'dark\\:'],
            },
            'typography': {
                'headings': ['heading-1', 'heading-2', 'heading-3', 'text-3xl', 'text-2xl', 'text-xl'],
                'body': ['body-text', 'body-small', 'text-base', 'text-sm'],
                'font_family': ['font-inter', 'Inter', 'font-family.*Inter'],
            },
            'spacing': {
                'base_8': ['space-1', 'space-2', 'space-4', 'space-8', 'space-16', 'space-24'],
                'padding': ['p-1', 'p-2', 'p-4', 'p-8', 'px-', 'py-'],
                'margin': ['m-1', 'm-2', 'm-4', 'm-8', 'mx-', 'my-'],
            },
            'components': {
                'buttons': ['btn-primary', 'btn-secondary', 'btn-tertiary', 'btn-danger', 'btn-success'],
                'cards': ['card-base', 'card-default', 'card-glass', 'card-premium', 'card-interactive'],
    ...
    def audit_color_consistency(self):
        ...
        for html_file in self.html_files:
            try:
                ...
                if len(inline_styles) > 5:  # Threshold
                    self.issues.append(f"  Excessive inline styles in {html_file} ({len(inline_styles)} found)")
        ...
        if inline_style_count > 20:
            self.issues.append("  Too many inline styles found - should use CSS classes instead")

    def audit_typography_consistency(self):
        """Audit typography usage consistency"""
        print("ù Auditing typography consistency...")
                ...
                if font_families:
                    for font in font_families:
                        if 'Inter' not in font and 'sans-serif' not in font:
                            self.issues.append(f"  Non-standard font found in {html_file}: {font}")
        ...
        if typography_usage['headings_heading-1'] < 1:
            self.issues.append("  Design system heading classes not being used consistently")

    def audit_spacing_consistency(self):
        """Audit spacing and layout consistency"""
        print("è Auditing spacing consistency...")
        ...
        for html_file in self.html_files:
            try:
                ...
                if len(arbitrary_spacing) > 3:
                    self.issues.append(f"  Too many arbitrary spacing values in {html_file} ({len(arbitrary_spacing)} found)")
    ...
    def audit_component_consistency(self):
        ...
        for html_file in self.html_files:
            try:
                ...
                button_elements = re.findall(r'<button[^>]*class=["\'][^"\']*["\'][^>]*>', content)
                for button in button_elements:
                    if not any(btn_class in button for btn_class in self.design_system['components']['buttons']):
                        if 'btn-' not in button:  # Should use design system
                            self.issues.append(f"  Button without design system class in {html_file}")
    ...
    def audit_dark_theme_consistency(self):
        ...
        print("ô Auditing dark theme consistency...")
        ...
        for html_file in self.html_files:
            try:
                ...
                light_bg_classes = re.findall(r'bg-white|bg-gray-50|bg-gray-100', content)
                for light_class in light_bg_classes:
                    if f"dark:{light_class.replace('white', 'gray-900').replace('50', '800').replace('100', '700')}" not in content:
    ...
    def audit_animation_performance(self):
        ...
        for html_file in self.html_files:
            try:
                ...
                if 'motion-reduce:' not in content and ('animate-' in content or 'transition-' in content):
                    motion_classes = len(re.findall(r'animate-|transition-', content))
                    if motion_classes > 3:
                        self.issues.append(f"  Missing motion-reduce variants in {html_file} (has {motion_classes} animations)")
            ...
            try:
                ...
                for element in required_pwa_elements:
                    if element not in content:
                        self.issues.append(f"  Missing PWA element in base template: {element}")
                    else:
    ...
    def generate_report(self) -> Dict[str, Any]:
        ...
        print("ä Generating audit report...")
        ...
        report = {
            'audit_summary': {
                'total_files_scanned': len(self.html_files) + len(self.css_files) + len(self.js_files),
    ...
    def generate_recommendations(self) -> List[str]:
        ...
        if self.stats.get('inline_styles', 0) > 20:
            recommendations.append("ß Reduce inline styles and move to CSS classes")

        if self.stats.get('brand_color_usage', 0) < 10:
            recommendations.append("® Increase usage of brand colors throughout the application")

        if len([issue for issue in self.issues if 'dark theme' in issue]) > 3:
            recommendations.append("ô Improve dark theme consistency across all components")

        if len([issue for issue in self.issues if 'Button without' in issue]) > 0:
            recommendations.append("© Apply design system button classes to all button elements")

        if len([issue for issue in self.issues if 'motion-reduce' in issue]) > 0:
            recommendations.append(" Add motion-reduce variants for better accessibility")
    ...
    def calculate_design_system_coverage(self) -> Dict[str, float]:
        ...
        for category in self.design_system:
            total_patterns = sum(len(patterns) for patterns in self.design_system[category].values())
            used_patterns = len([k for k in self.stats.get(f'{category}_usage', {}) if self.stats.get(f'{category}_usage', {}).get(k, 0) > 0])
            coverage[category] = (used_patterns / total_patterns * 100) if total_patterns > 0 else 0
...
def main():
    ...
    print(f"\nÑ Full report saved to: {report_file}")
