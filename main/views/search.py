...
from main.search import search_engine
...
class TagCloudView(TemplateView):
    ...
    def _collect_all_tags(self):
        ...
        from main.models import Project, AITool
        ...
        try:
            ...
            for tool in tools:
                if hasattr(tool, 'tags') and tool.tags:
                    tags = tool.tags.split(',') if isinstance(tool.tags, str) else tool.tags
                    for tag in tags:
                        if tag.strip():
    ...
    def _add_tag(self, tag_data, tag, category, obj):
        ...
        tag_data[tag_lower]['items'].append({
            'type': category,
            'object': obj
        })
    ...
    def _generate_tag_cloud(self, tag_data):
        ...
        for tag_key, tag_info in sorted_tags[:100]:  # Limit to top 100 tags
            ...
            if max_count == min_count:
                ...
                size = 1 + int(4 * (tag_info['count'] - min_count) / (max_count - min_count))
...
def search_by_tag(request, tag):
    ...
    filtered_results = []
    for result in search_results['results']:
        if any(tag.lower() in result_tag.lower() for result_tag in result.get('tags', [])):
