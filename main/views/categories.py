...
from main.models import (
    BlogCategory, Project, AITool
)
from main.filters import BlogFilter, ProjectFilter, ContentFilter
    ...
    def get_context_data(self, **kwargs):
        ...
        if date_range:
            if date_range == 'custom':
                ...
                from main.filters import apply_date_range_filter
                posts_queryset = apply_date_range_filter(posts_queryset, date_range, 'published_at')
                blog_filter = BlogFilter(posts_queryset)
        
        # Sort posts
        sort_by = self.request.GET.get('sort', 'published_at')
        sort_direction = 'desc' if sort_by.startswith('-') else 'desc'  # Default to desc for dates
...
class AIToolCategoryView(ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context.update({
            ...
            'filter_options': {
                'categories': [{'value': c[0], 'label': c[1]} for c in AITool.CATEGORY_CHOICES],
                'sort_options': [
    ...
    def get_context_data(self, **kwargs):
        ...
        stats = {
            ...
            'active_projects': Project.objects.filter(
                is_visible=True, 
                status__in=['development', 'testing']
            ).count(),
        ...
        context.update({
            ...
            'filter_options': {
                'statuses': [{'value': s[0], 'label': s[1]} for s in Project.STATUS_CHOICES],
                'difficulty_levels': [
                    {'value': 1, 'label': 'Kolay'},
                    {'value': 2, 'label': 'Orta'},
                    {'value': 3, 'label': 'Zor'},
