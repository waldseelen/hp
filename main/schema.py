...
class StructuredData:
    ...
    def website_schema(request):
        ...
        return {
            ...
            "potentialAction": {
                ...
                "target": {
                    "@type": "EntryPoint",
                    "urlTemplate": request.build_absolute_uri('/') + "?q={search_term_string}"
                },
    ...
    def blog_post_schema(request, post):
        ...
        return {
            ...
            "mainEntityOfPage": {
                "@type": "WebPage",
                "@id": request.build_absolute_uri(reverse('blog:detail', args=[post.slug]))
            },
            "publisher": {
                "@type": "Organization", 
                "name": "Portfolio",
            ...
            "image": {
                "@type": "ImageObject",
                "url": post.featured_image.url if post.featured_image else request.build_absolute_uri('/static/images/og-image.png'),
                "width": 1200,
                "height": 630
            } if hasattr(post, 'featured_image') else {
                "@type": "ImageObject",
    ...
    def project_schema(request, project):
        ...
        return {
            ...
            "url": request.build_absolute_uri(reverse('main:project_detail', args=[project.slug])),
            ...
            "dateCreated": project.created_at.isoformat() if hasattr(project, 'created_at') else timezone.now().isoformat(),
            "dateModified": project.updated_at.isoformat() if project.updated_at else timezone.now().isoformat(),
            "programmingLanguage": project.technologies.split(',') if hasattr(project, 'technologies') and project.technologies else ["Python", "Django"],
            "codeRepository": project.github_url if hasattr(project, 'github_url') and project.github_url else None,
            "screenshot": {
                "@type": "ImageObject",
                "url": project.image.url if hasattr(project, 'image') and project.image else request.build_absolute_uri('/static/images/project-placeholder.png'),
                "width": 1200,
                "height": 630
            }
        }
    ...
    def breadcrumb_schema(request, items):
        ...
        for i, item in enumerate(items, 1):
            list_items.append({
                ...
                "position": i,
                "name": item['name'],
                "item": request.build_absolute_uri(item['url']) if item.get('url') else None
            })
