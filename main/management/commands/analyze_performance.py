...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(f'Analysis Time: {lighthouse_result.get("fetchTime", "Unknown")}')

        # Display category scores
        self.stdout.write('\n--- Category Scores ---')
        for category_name, category_data in categories.items():
            score = category_data.get('score', 0) * 100 if category_data.get('score') is not None else 0
        ...
        for audit_id, audit_title in core_vitals.items():
            ...
            if audit:
                ...
                if score is not None:
                    ...
                    self.stdout.write(f'{style(audit_title)}: {display_value} ({score_pct:.0f}/100)')
                else:
        ...
        if performance_score < 50:
            self.stdout.write(self.style.ERROR('Performance needs immediate attention:'))
            ...
            self.stdout.write(self.style.SUCCESS('Great performance! Keep monitoring regularly.'))
