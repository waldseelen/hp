...
from main.models import Project, BlogCategory
...
class ContentFilter:
    ...
    def filter_by_category(self, categories):
        ...
        if category_field:
            if hasattr(self.model._meta.get_field(category_field), 'related_model'):
                ...
                self.queryset = self.queryset.filter(**{f"{category_field}__name__in": categories})
            else:
                # Direct field
                self.queryset = self.queryset.filter(**{f"{category_field}__in": categories})
    ...
    def filter_by_difficulty(self, min_difficulty=None, max_difficulty=None):
        ...
        if hasattr(self.model, 'difficulty_level'):
            if min_difficulty:
                self.queryset = self.queryset.filter(difficulty_level__gte=min_difficulty)
            if max_difficulty:
                self.queryset = self.queryset.filter(difficulty_level__lte=max_difficulty)
    ...
    def _get_sortable_fields(self):
        ...
        sortable_fields_map = {
            ...
            'Project': base_fields + ['title', 'view_count', 'difficulty_level', 'start_date'],
    ...
    def filter_by_reading_time(self, min_time=None, max_time=None):
        ...
        if min_time:
            self.queryset = self.queryset.filter(
                reading_time__gte=min_time
            ) if hasattr(self.model, 'reading_time') else self.queryset
        
        if max_time:
            self.queryset = self.queryset.filter(
                reading_time__lte=max_time
            ) if hasattr(self.model, 'reading_time') else self.queryset
    ...
    def filter_by_completion_date(self, completed_after=None, completed_before=None):
        ...
        if completed_before:
            if isinstance(completed_before, str):
                completed_before = datetime.strptime(completed_before, '%Y-%m-%d').date()
...
def get_filter_options():
    ...
    from main.models import AITool, CybersecurityResource, UsefulResource
    
    # Get categories for different content types
    blog_categories = []
    try:
        blog_categories = list(BlogCategory.objects.filter(
            is_visible=True
