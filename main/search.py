    ...
    def _search_model(self, config, keywords, query):
        ...
        if tag_field:
            for keyword in keywords:
                ...
                if hasattr(model.objects.first(), tag_field) if model.objects.exists() else False:
        ...
        try:
            ...
            results = []
            for obj in queryset:
                score = self._calculate_relevance_score(obj, keywords, fields, tag_field, weight)
                if score > 0:
