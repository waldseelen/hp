...
class SearchEngine:
    ...
    def _calculate_field_score(self, field, value, keywords, field_weights):
        ...
        for keyword in keywords:
            ...
            if keyword_lower in value_lower:
                ...
                if field in ["title", "name"]:
                    score += (
                        weight * 20 if keyword_lower == value_lower else weight * 10
                    )
                elif re.search(rf"\b{re.escape(keyword_lower)}\b", value_lower):
                    score += weight * 5
                else:
                    score += weight * 2
    ...
    def _calculate_tag_score(self, obj, keywords, tag_field):
    ...
    def _apply_boost_multipliers(self, score, obj):
        """Apply boost multipliers for featured and recent items."""
            ...
            try:
                ...
                if isinstance(value, str):
                    score += self._calculate_field_score(
                        field, value, keywords, field_weights
                    )
            except (AttributeError, TypeError):
                continue

        # Bonus for tag matches
        score += self._calculate_tag_score(obj, keywords, tag_field)

        # Apply base model weight
        score *= base_weight / 10

        # Apply boost multipliers
        score = self._apply_boost_multipliers(score, obj)
