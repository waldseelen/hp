...
def health_check_endpoint(request):
    ...
    try:
        ...
        overall_healthy = all(
            ...
            for check in basic_checks.values()
        )
...
def readiness_probe(request):
    ...
    try:
        ...
        ready = (
            db_check['status'] in ['healthy', 'warning'] and
            cache_check['status'] in ['healthy', 'warning']
        )
        ...
        for check_name, check_result in results['checks'].items():
            ...
            if 'details' in check_result:
                ...
                if check_name == 'database' and 'query_time_ms' in details:
                    metrics.append(f'health_database_query_time_ms {details["query_time_ms"]}')
                
                if check_name == 'cache' and 'response_time_ms' in details:
                    metrics.append(f'health_cache_response_time_ms {details["response_time_ms"]}')
                
                if check_name == 'disk_space' and 'free_percent' in details:
                    metrics.append(f'health_disk_free_percent {details["free_percent"]}')
                
                if check_name == 'memory' and 'used_percent' in details:
                    metrics.append(f'health_memory_used_percent {details["used_percent"]}')
        
        # Uptime metrics
        if uptime_stats['status'] != 'error':
            metrics.append(f'health_uptime_percentage {uptime_stats.get("uptime_percentage", 0)}')
            metrics.append(f'health_checks_total_today {uptime_stats.get("total_checks_today", 0)}')
            metrics.append(f'health_checks_healthy_today {uptime_stats.get("healthy_checks", 0)}')
            metrics.append(f'health_checks_unhealthy_today {uptime_stats.get("unhealthy_checks", 0)}')
...
# Import required modules at the top level for the views
import django
import sys