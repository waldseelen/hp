...
def cache_stats_api(request):
    ...
    try:
        ...
        stats.update({
            ...
            'redis_available': 'redis' in settings.CACHES['default']['BACKEND'].lower(),
        })
...
def cache_monitor_dashboard(request):
    ...
    try:
        ...
        uptime_hours = stats['uptime_seconds'] / 3600 if stats['uptime_seconds'] > 0 else 0.01

        dashboard_data = {
            'overview': {
                'hit_ratio': stats['hit_ratio'],
                'total_operations': total_ops,
                'operations_per_hour': total_ops / uptime_hours if uptime_hours > 0 else 0,
                'error_rate': (stats['errors'] / total_ops * 100) if total_ops > 0 else 0,
...
def get_performance_grade(hit_ratio):
    ...
    if hit_ratio >= 95:
        return {'grade': 'A+', 'color': 'success', 'description': 'Excellent performance'}
    elif hit_ratio >= 85:
        return {'grade': 'A', 'color': 'success', 'description': 'Very good performance'}
    elif hit_ratio >= 75:
        return {'grade': 'B', 'color': 'warning', 'description': 'Good performance'}
    elif hit_ratio >= 65:
        return {'grade': 'C', 'color': 'warning', 'description': 'Acceptable performance'}
    else:
        return {'grade': 'D', 'color': 'danger', 'description': 'Poor performance - needs optimization'}
...
def get_cache_recommendations(stats):
    ...
    recommendations = []
    hit_ratio = stats['hit_ratio']
    error_rate = (stats['errors'] / stats['total_operations'] * 100) if stats['total_operations'] > 0 else 0
    ...
    if stats['total_operations'] < 100 and stats['uptime_seconds'] > 3600:  # Low usage after 1 hour
        recommendations.append({
            'type': 'usage',
    ...
    try:
        ...
        metrics = {
            'hit_ratio_timeline': [
                {
                    ...
                    'hit_ratio': max(0, current_stats['hit_ratio'] + (i * 2 - 10)),  # Mock variation
                }
            ...
            'operations_timeline': [
                {
                    'timestamp': (now - timezone.timedelta(minutes=i*5)).isoformat(),
                    'operations': max(0, current_stats['total_operations'] // 12 + (i * 5)),
                }
                for i in range(12)
            ],
...
def cache_invalidate_webhook(request):
    ...
    try:
        ...
        if pattern:
            ...
            return JsonResponse({
                'status': 'error',
                'message': 'No invalidation parameters provided'
            }, status=400)
