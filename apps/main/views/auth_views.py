...
from apps.main.models import UserSession, Admin
from apps.main.auth_backends import get_client_ip, get_device_info
...
def terminate_all_sessions(request):
    ...
    terminated_count = user.sessions.filter(
        is_active=True
    ).exclude(
...
def security_dashboard(request):
    ...
    recent_sessions = user.sessions.filter(is_active=True).order_by('-last_activity')[:10]
...
def two_factor_login(request):
    ...
    if not totp_token and not backup_code:
        messages.error(request, 'Please enter either a verification code or backup code.')
