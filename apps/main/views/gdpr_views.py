    ...
    try:
        ...
        if export_format == 'json':
            ...
            with zipfile.ZipFile(zip_buffer, 'w', zipfile.ZIP_DEFLATED) as zip_file:
                ...
                # Add CSV summary
                csv_output = StringIO()
                csv_output.write(f"User Data Export Summary\n")
                csv_output.write(f"Username: {user.username}\n")
                csv_output.write(f"Email: {user.email}\n")
                csv_output.write(f"Export Date: {timezone.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
                csv_output.write(f"Total Contact Messages: {len(export_data['contact_messages'])}\n")
                csv_output.write(f"Total Chat Messages: {len(export_data['chat_messages'])}\n")

                zip_file.writestr(f'user_data_summary_{user.id}.csv', csv_output.getvalue())
...
"""
            ...
            response['Content-Disposition'] = f'attachment; filename="user_data_{user.id}.zip"'
    ...
    try:
        ...
        with transaction.atomic():
            ...
            if deletion_type == 'soft':
                ...
                messages.success(request, 'Your account has been anonymized successfully.')
    ...
    try:
        ...
        if request.method == 'POST':
            ...
            if deletion_request.user.check_password(password):
                ...
                # Backup user data
                deletion_request.user_data_backup = generate_user_data_export(deletion_request.user)
        ...
        return render(request, 'gdpr/confirm_deletion.html', {
...
def send_export_email(user, email, filename):
    ...
    message = render_to_string('emails/data_export_ready.txt', {
        'user': user,
        'filename': filename,
    })
...
def send_deletion_confirmation_email(user, token):
    ...
    confirmation_url = f"{settings.SITE_URL}{reverse('confirm_account_deletion', args=[token])}"

    message = render_to_string('emails/deletion_confirmation.txt', {
        'user': user,
...
def cookie_consent_status(request):
    ...
    consent_preferences = request.session.get('cookie_consent', {
        'necessary': True,
        'functional': False,
        'analytics': False,
        'marketing': False,
