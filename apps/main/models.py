...
class PerformanceMetric(models.Model):
    ...
    timestamp = models.DateTimeField(
        ...
        help_text="When the metric was collected"
    )
    created_at = models.DateTimeField(default=timezone.now)
    ...
    def is_good_score(self):
        ...
        if self.metric_type == 'lcp':
            ...
            return self.value <= thresholds.get('FID_THRESHOLD', 0.1) * 1000  # Convert to ms
        elif self.metric_type == 'cls':
            return self.value <= thresholds.get('CLS_THRESHOLD', 0.1)
        elif self.metric_type == 'inp':
            return self.value <= thresholds.get('INP_THRESHOLD', 0.2) * 1000  # Convert to ms
        elif self.metric_type == 'ttfb':
            return self.value <= thresholds.get('TTFB_THRESHOLD', 0.8) * 1000  # Convert to ms
        return None
...
class WebPushSubscription(models.Model):
    ...
    p256dh = models.TextField(
        help_text="P-256 ECDH public key (base64 encoded)"
    )
    ...
    ip_address = models.GenericIPAddressField(
        blank=True,
        null=True,
        help_text="IP address when subscription was created"
    )
    country_code = models.CharField(
        max_length=2,
        blank=True,
    ...
    total_sent = models.IntegerField(
        default=0,
        help_text="Total notifications sent to this subscription"
    )
    total_delivered = models.IntegerField(
        default=0,
        help_text="Total notifications successfully delivered"
    )
    total_failed = models.IntegerField(
        default=0,
        help_text="Total failed delivery attempts"
    )
    last_success = models.DateTimeField(
        blank=True,
        null=True,
        help_text="Last successful notification delivery"
    )
    last_failure = models.DateTimeField(
        blank=True,
        null=True,
        help_text="Last failed notification delivery"
    )
    ...
    def __str__(self):
        ...
        return f"Push Subscription{browser_info} - {self.created_at.strftime('%Y-%m-%d')}"
...
class NotificationLog(models.Model):
    ...
    image = models.URLField(
        blank=True,
        help_text="Notification image URL"
    )
...
class ErrorLog(models.Model):
    ...
    url = models.URLField(
        blank=True,
        max_length=500,
        help_text="URL where error occurred"
    )
    user_agent = models.TextField(
        blank=True,
        help_text="Browser user agent string"
    ...
    file_name = models.CharField(
        max_length=255,
        blank=True,
        help_text="Source file where error occurred"
    )
    ...
    resolved_at = models.DateTimeField(
        blank=True,
        null=True,
        help_text="When error was marked as resolved"
    )
...
class ShortURL(models.Model):
    ...
    short_code = models.CharField(
        max_length=10,
        unique=True,
        help_text="Short code for the URL"
    )
