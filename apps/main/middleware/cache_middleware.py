...
class APIResponseCacheMiddleware(MiddlewareMixin):
    ...
    def __init__(self, get_response):
        ...
        self.cacheable_paths = getattr(settings, 'API_CACHEABLE_PATHS', [
            '/api/posts/',
            '/api/personal-info/',
            '/api/social-links/',
            '/api/ai-tools/',
...
class CacheHeadersMiddleware(MiddlewareMixin):
    ...
    def process_response(self, request, response):
        ...
        if request.path.startswith('/admin/') or (hasattr(settings, 'DEBUG') and settings.DEBUG):
            return response
        ...
        # API endpoints
            ...
            if response.status_code == 200 and request.method == 'GET':
                response['Cache-Control'] = f'public, max-age={self.dynamic_cache_timeout}'
                response['Expires'] = self.get_expires_header(self.dynamic_cache_timeout)
            else:
...
class RequestTimingMiddleware(MiddlewareMixin):
    ...
    def store_api_performance_metric(self, request, response, duration):
        ...
        try:
            ...
            if response.status_code == 200 and hash(request.path) % 10 == 0:  # 10% sampling
...
def cache_api_response(timeout=300, vary_on=None, key_func=None):
