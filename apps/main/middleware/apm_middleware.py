...
try:
    import sentry_sdk
    from sentry_sdk import start_transaction, set_tag, set_context
...
class APMMiddleware(MiddlewareMixin):
    ...
    def process_request(self, request: HttpRequest) -> Optional[HttpResponse]:
        ...
        set_tag("user.authenticated", request.user.is_authenticated if hasattr(request, 'user') else False)
    ...
    def _check_performance_budgets(self, request: HttpRequest, response: HttpResponse, response_time: float):
...
class DatabaseQueryTrackingMiddleware(MiddlewareMixin):
    ...
    def process_request(self, request: HttpRequest) -> Optional[HttpResponse]:
        ...
        if SENTRY_AVAILABLE and hasattr(settings, 'PERFORMANCE_MONITORING') and settings.PERFORMANCE_MONITORING.get('TRACK_SQL_QUERIES', False):
    ...
    def process_response(self, request: HttpRequest, response: HttpResponse) -> HttpResponse:
        ...
        if (SENTRY_AVAILABLE and
            hasattr(request, '_apm_initial_queries') and
            hasattr(request, '_apm_query_start_time')):

            from django.db import connection

            query_count = len(connection.queries) - request._apm_initial_queries
            query_time = sum(float(query['time']) for query in connection.queries[request._apm_initial_queries:])

            # Add database context
            set_context("database", {
                "query_count": query_count,
                "query_time_ms": round(query_time * 1000, 2),
                "queries": connection.queries[request._apm_initial_queries:] if settings.DEBUG else []
            })
