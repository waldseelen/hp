...
class Command(BaseCommand):
    ...
    def check_application(self):
        ...
        try:
            ...
            from apps.main.models import PersonalInfo, Admin
    ...
    def display_results(self, results):
        ...
        status_color = self.style.SUCCESS if results['status'] == 'healthy' else self.style.ERROR
        ...
        for check_name, check_result in results['checks'].items():
            status_style = self.style.SUCCESS if check_result['status'] == 'healthy' else self.style.ERROR
            self.stdout.write(f"{check_name.replace('_', ' ').title()}: {status_style(check_result['status'])}")
