...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if self.once_mode:
            ...
            self.stdout.write(f"Starting health monitoring daemon (interval: {self.interval}s)")
            self._run_daemon()
        else:
            self.stdout.write(f"Running health monitoring for {self.interval}s intervals. Press Ctrl+C to stop.")
        ...
        if results['overall_status'] != 'healthy':
            ...
            for check_name, check_result in results['checks'].items():
                if check_result['status'] != 'healthy':
                    status_icon = 'WARN' if check_result['status'] == 'warning' else 'FAIL'
                    self.stdout.write(
                        f"  {status_icon} {check_name}: {check_result.get('message', 'No message')}"
                    )
                    
                    # Show additional details if available
                    if 'details' in check_result and 'issues' in check_result['details']:
    ...
    def _show_final_stats(self):
        ...
        if self.stats['checks_run'] > 0:
            uptime_percent = (self.stats['healthy_count'] / self.stats['checks_run']) * 100
