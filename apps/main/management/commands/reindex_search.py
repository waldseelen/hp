    ...
    def handle(self, *args, **options):
        ...
        if options['all']:
            ...
            raise CommandError(
                ...
                'Available models: ' + ', '.join(search_index_manager.model_registry.keys())
            )
        ...
        for model_name in models_to_index:
            ...
            try:
                ...
                if results['indexed'] > 0:
                    self.stdout.write(self.style.SUCCESS(f'   ✓ Indexed: {results["indexed"]}'))
                ...
                if options['verbose'] and results['indexed'] > 0:
                    ...
                    self.stdout.write(f'   ⏱ Duration: {model_duration:.2f}s ({docs_per_sec:.1f} docs/sec)')
        ...
        self.stdout.write(self.style.SUCCESS(f'✓ Indexed:  {total_results["indexed"]:,}'))
        if total_results['skipped'] > 0:
            self.stdout.write(self.style.WARNING(f'⚠ Skipped:  {total_results["skipped"]:,}'))
        ...
        try:
            stats = search_index_manager.get_index_stats()
            self.stdout.write(f'\n📈 Final index stats:')
            self.stdout.write(f'   Documents in index: {stats.get("number_of_documents", 0):,}')
            self.stdout.write(f'   Indexing in progress: {stats.get("is_indexing", False)}')
        except Exception as e:
