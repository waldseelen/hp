    ...
    def items(self):
        ...
        for post in recent_posts:
            items.append({
                ...
                'title': post.title,
                'description': post.excerpt if post.excerpt else post.content[:200] + '...',
                'link': reverse('blog:detail', args=[post.slug]),
                'guid': f"post-{post.id}",
                'categories': post.tags if hasattr(post, 'tags') and post.tags else []
            })
        ...
        for project in recent_projects:
            items.append({
                ...
                'categories': [project.get_status_display()] + (project.tech_stack if project.tech_stack else [])
            })
