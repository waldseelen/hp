...
class SEOOptimizer:
    ...
    def generate_meta_description(content, max_length=155):
        ...
        if len(clean_content) > max_length:
            ...
            description = ""
            for word in words:
                if len(description + word + ' ') <= max_length - 3:  # Leave space for '...'
                    description += word + ' '
                else:
                    break
    ...
    def generate_alt_text(filename, context=""):
        ...
        base_name = re.sub(r'^(img_|image_|pic_|photo_)', '', base_name, flags=re.IGNORECASE)
        base_name = re.sub(r'_(thumb|thumbnail|small|medium|large|xl)$', '', base_name, flags=re.IGNORECASE)
