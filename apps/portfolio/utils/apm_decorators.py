...
try:
    import sentry_sdk
    from sentry_sdk import start_span, set_tag, set_context
...
def trace_function(operation_name: str = None, description: str = None, tags: Dict[str, Any] = None):
    """
    ...
    def decorator(func: Callable) -> Callable:
        ...
        def wrapper(*args, **kwargs):
            ...
            # Start span
            with start_span(op=op_name, description=desc) as span:
                ...
                try:
                    ...
                    set_tag("performance.execution_time_ms", round(execution_time * 1000, 2))
...
def trace_database_operation(operation_type: str = "database"):
    ...
    def decorator(func: Callable) -> Callable:
        ...
        def wrapper(*args, **kwargs):
            ...
            with start_span(op=f"db.{operation_type}", description=f"Database {operation_type}: {func.__name__}") as span:
                ...
                try:
                    ...
                    set_context("database_operation", {
                    ...
                    if hasattr(settings, 'PERFORMANCE_BUDGETS'):
                        db_threshold = settings.PERFORMANCE_BUDGETS.get('DATABASE_QUERY_THRESHOLD', 0.1)
...
def trace_cache_operation(operation_type: str = "cache"):
    ...
    def decorator(func: Callable) -> Callable:
        ...
        def wrapper(*args, **kwargs):
            ...
            with start_span(op=f"cache.{operation_type}", description=f"Cache {operation_type}: {func.__name__}") as span:
                ...
                try:
                    ...
                    set_context("cache_operation", {
                        "operation_type": operation_type,
                        "execution_time_ms": round(execution_time * 1000, 2),
                        "cache_hit": getattr(result, 'cache_hit', None) if hasattr(result, 'cache_hit') else None,
                    })
                    ...
                    if hasattr(settings, 'PERFORMANCE_BUDGETS'):
                        cache_threshold = settings.PERFORMANCE_BUDGETS.get('CACHE_OPERATION_THRESHOLD', 0.05)
...
def trace_api_call(service_name: str, endpoint: str = None):
    ...
    def decorator(func: Callable) -> Callable:
        ...
        def wrapper(*args, **kwargs):
            ...
            with start_span(op="http.client", description=f"API call to {service_name}: {endpoint_name}") as span:
                ...
                try:
                    ...
                    if hasattr(settings, 'PERFORMANCE_BUDGETS'):
                        api_threshold = settings.PERFORMANCE_BUDGETS.get('API_RESPONSE_THRESHOLD', 1.0)
...
class APMContext:
    ...
    def __init__(self, operation_name: str, description: str = None, tags: Dict[str, Any] = None):
