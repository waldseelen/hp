...
from apps.main.models import PersonalInfo, SocialLink, BlogCategory
...
class Command(BaseCommand):
    ...
    def _warm_home_data(self, force: bool, timeout: Optional[int], verbose: bool, stats: Dict):
        """Warm cache data for home page"""
        if verbose:
            self.stdout.write('Warming home page data...')
        
        warming_functions = [
            ('personal_info_visible', self._get_personal_info, CacheManager.TIMEOUTS['long']),
            ('social_links_visible', self._get_social_links, CacheManager.TIMEOUTS['long']),
            ('recent_posts', self._get_recent_posts, CacheManager.TIMEOUTS['medium']),
            ('featured_tools', self._get_featured_tools, CacheManager.TIMEOUTS['medium']),
            ('featured_blog_categories', self._get_featured_blog_categories, CacheManager.TIMEOUTS['medium']),
        ]
    ...
    def _warm_blog_data(self, force: bool, timeout: Optional[int], verbose: bool, stats: Dict):
        """Warm cache data for blog"""
        if verbose:
            self.stdout.write('Warming blog data...')
        
        warming_functions = [
            ('published_posts', self._get_published_posts, CacheManager.TIMEOUTS['medium']),
            ('popular_posts', self._get_popular_posts, CacheManager.TIMEOUTS['medium']),
            ('blog_categories', self._get_blog_categories, CacheManager.TIMEOUTS['long']),
        ]
    ...
    def _warm_tools_data(self, force: bool, timeout: Optional[int], verbose: bool, stats: Dict):
        ...
        warming_functions = [
            ...
            ('featured_tools', self._get_featured_tools, CacheManager.TIMEOUTS['medium']),
            ('tools_by_category', self._get_tools_by_category, CacheManager.TIMEOUTS['medium']),
        ]
    ...
    def _warm_personal_data(self, force: bool, timeout: Optional[int], verbose: bool, stats: Dict):
        """Warm cache data for personal page"""
        if verbose:
            self.stdout.write('Warming personal data...')
        
        warming_functions = [
            ('personal_info_all', self._get_personal_info_all, CacheManager.TIMEOUTS['long']),
    ...
    def _warm_single_entry(self, cache_key: str, data_func, timeout: int, 
                          force: bool, verbose: bool, stats: Dict):
    ...
    def _get_social_links(self):
        ...
        return list(SocialLink.objects.filter(is_visible=True).order_by('order', 'platform'))
    
    def _get_recent_posts(self):
        """Get recent published posts"""
        return list(Post.objects.select_related('author')
    ...
    def _get_visible_tools(self):
        ...
        return list(Tool.objects.filter(is_visible=True)
                   .order_by('-is_favorite', 'order', '-created_at'))
    ...
    def _get_blog_categories(self):
        """Get blog categories"""
        return list(BlogCategory.objects.filter(is_visible=True)
                   .order_by('order', 'name'))
