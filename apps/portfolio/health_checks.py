...
class HealthCheckSystem:
    ...
    def check_database(self) -> Dict:
        ...
        try:
            ...
            return {
                'status': status,
                'message': 'Database connection healthy' if status == 'healthy' else 'Database has issues',
                'details': {
    ...
    def check_cache(self) -> Dict:
        ...
        try:
            ...
            return {
                'status': status,
                'message': 'Cache system healthy' if status == 'healthy' else 'Cache has issues',
                'details': {
    ...
    def check_disk_space(self) -> Dict:
        ...
        try:
            ...
            return {
                'status': status,
                'message': 'Disk space healthy' if status == 'healthy' else 'Disk space issues',
                'details': {
    ...
    def check_memory(self) -> Dict:
        ...
        try:
            ...
            return {
                'status': status,
                'message': 'Memory usage healthy' if status == 'healthy' else 'Memory usage issues',
                'details': {
                    'total_gb': round(total_gb, 2),
    ...
    def check_external_services(self) -> Dict:
        ...
        return {
            ...
            'details': {
                ...
                'healthy_count': len([r for r in results if r.get('status') == 'healthy'])
            },
            'timestamp': timezone.now().isoformat()
        }
    ...
    def check_application_health(self) -> Dict:
        ...
        try:
            ...
            return {
                'status': status,
                'message': 'Application healthy' if status == 'healthy' else 'Application has configuration issues',
                'details': {
    ...
    def check_security_status(self) -> Dict:
        ...
        try:
            ...
            return {
                'status': status,
                'message': 'Security configuration healthy' if status == 'healthy' else 'Security configuration has issues',
                'details': {
    ...
    def _send_health_alerts(self, results: Dict):
        ...
        try:
            ...
            for check_name, check_result in results['checks'].items():
                if check_result['status'] == 'error':
                    failed_checks.append(f"FAIL {check_name}: {check_result.get('message', 'Failed')}")
                elif check_result['status'] == 'warning':
                    warning_checks.append(f"WARN {check_name}: {check_result.get('message', 'Warning')}")
...
Details: {json.dumps(details, indent=2) if details else 'None'}
    ...
    def get_uptime_stats(self) -> Dict:
        ...
        try:
            ...
            healthy_checks = len([h for h in today_history if h['overall_status'] == 'healthy'])
            warning_checks = len([h for h in today_history if h['overall_status'] == 'warning'])
            unhealthy_checks = len([h for h in today_history if h['overall_status'] == 'unhealthy'])
