...
class RateLimitMiddleware(MiddlewareMixin):
    ...
    def process_request(self, request):
        ...
        if settings.DEBUG and not getattr(settings, 'ENABLE_RATE_LIMIT_IN_DEBUG', False):
            return None
        ...
        if not is_allowed:
            ...
            response['X-RateLimit-Limit'] = str(self.get_limit_for_path(request.path)[0])
    ...
    def process_response(self, request, response):
        ...
        if hasattr(request, 'rate_limit_remaining'):
            ...
            response['X-RateLimit-Reset'] = str(int(time.time() + request.rate_limit_reset))
...
class APIRateLimitMiddleware(MiddlewareMixin):
    ...
    def process_response(self, request, response):
        ...
        if hasattr(request, 'api_rate_limit_remaining'):
            ...
            response['X-API-RateLimit-Remaining'] = str(request.api_rate_limit_remaining)
...
def ratelimit_view(key='ip', rate='10/m', method='ALL', block=True):
    ...
    def decorator(view_func):
        def wrapped_view(request, *args, **kwargs):
            ...
            response['X-View-RateLimit-Remaining'] = str(max(0, limit - current_count - 1))
