...
def track_funnel(request):
    ...
    try:
        ...
        if not all([funnel_name, step_name]):
            return JsonResponse({'error': 'funnel_name and step_name required'}, status=400)
        
        success = analytics.track_funnel_step(request, funnel_name, step_name, step_order)
...
def track_ab_conversion(request):
    ...
    try:
        ...
        success = analytics.track_ab_test_conversion(request, test_name, conversion_type)
