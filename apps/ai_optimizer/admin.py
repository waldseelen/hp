...
from .models import OptimizationSession, OptimizationResult, AIOptimizerSettings, OptimizationMetrics
...
class AIOptimizerSettingsAdmin(admin.ModelAdmin):
    ...
    list_display = [
        'enabled', 'auto_optimization', 'min_confidence_threshold',
        'max_sessions_per_hour', 'updated_at'
    ]
    
    fieldsets = [
        ('Feature Controls', {
            'fields': ['enabled', 'auto_optimization']
        }),
        ('Analysis Modules', {
            'fields': [
                'image_analysis_enabled', 'content_analysis_enabled',
                'performance_prediction_enabled', 'accessibility_analysis_enabled'
            ]
        }),
        ('Thresholds & Limits', {
            'fields': ['min_confidence_threshold', 'max_sessions_per_hour', 'cache_timeout']
        }),
        ('External APIs', {
            'fields': ['external_apis'],
            'classes': ['collapse']
        }),
        ('Timestamps', {
...
class OptimizationMetricsInline(admin.TabularInline):
    ...
    fields = [
        ...
        'improvement_percent', 'measured_at'
    ]
...
class OptimizationSessionAdmin(admin.ModelAdmin):
    ...
    list_display = [
        'session_id_short', 'optimization_type', 'status',
        'target_url_short', 'started_at', 'duration_display',
        'results_count'
    ]
    
    list_filter = [
        'status', 'optimization_type', 'started_at'
    ]
    
    search_fields = [
        'session_id', 'target_url', 'optimization_type'
    ]
    
    readonly_fields = [
        'session_id', 'started_at', 'completed_at', 'duration_display',
        'results_count', 'config_display', 'results_display'
    ]
    
    fieldsets = [
        ('Session Info', {
            'fields': [
                'session_id', 'optimization_type', 'status', 'target_url'
            ]
        }),
        ('Content Object', {
    ...
    def results_count(self, obj):
        ...
        if count > 0:
            ...
            return format_html(
                '<a href="{}?session__id__exact={}">{} results</a>',
                url, obj.id, count
            )
        return '0 results'
...
class OptimizationResultAdmin(admin.ModelAdmin):
    ...
    list_display = [
        'title_short', 'category', 'priority', 'status',
        'impact_score', 'confidence_score', 'session_link',
        'created_at'
    ]
    ...
    search_fields = [
        'title', 'description', 'session__session_id'
    ]
    
    readonly_fields = [
        'session', 'created_at', 'updated_at', 'metrics_count',
        'before_metrics_display', 'after_metrics_display'
    ]
    
    fieldsets = [
        ('Basic Info', {
            'fields': [
                'session', 'category', 'title', 'description'
            ]
        }),
        ('Status & Priority', {
        ...
        ('Scores', {
            'fields': ['impact_score', 'confidence_score']
        }),
        ('Metrics', {
            'fields': [
                'metrics_count', 'before_metrics_display', 'after_metrics_display'
            ],
            'classes': ['collapse']
        }),
        ('Timestamps', {
    ...
    def session_link(self, obj):
        url = reverse('admin:ai_optimizer_optimizationsession_change', args=[obj.session.id])
...
class OptimizationMetricsAdmin(admin.ModelAdmin):
    ...
    list_display = [
        'optimization_result_short', 'metric_type', 'before_value',
        'after_value', 'improvement_percent', 'measured_at'
    ]
    ...
    search_fields = [
        'optimization_result__title', 'metric_type'
    ]
