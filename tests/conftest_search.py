...
def mock_search_index_manager(mock_meilisearch_client):
    ...
    with patch('apps.main.search_index.search_index_manager') as mock_manager:
        ...
        mock_manager.bulk_index.return_value = {
            'indexed': 0,
            'skipped': 0,
            'failed': 0
        }
...
def pytest_configure(config):
    ...
    config.addinivalue_line(
        "markers", "unit: Unit tests for search functionality"
    )
    config.addinivalue_line(
        "markers", "integration: Integration tests for search"
    )
    config.addinivalue_line(
        "markers", "search: All search-related tests"
    )
    config.addinivalue_line(
        "markers", "api: API endpoint tests"
    )
    config.addinivalue_line(
        "markers", "admin: Admin interface tests"
    )
    config.addinivalue_line(
        "markers", "performance: Performance tests"
    )
    config.addinivalue_line(
        "markers", "security: Security tests (XSS, sanitization)"
    )
